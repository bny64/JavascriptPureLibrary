...
export default {
    template: `
        <div class="page-wrap" style="max-width: 100%;">
            ...
            <div class="toolbar">
                ...
                <span style="font-size:13px; color:var(--muted)">개 상품 (Manual validate)</span>
            ...
            <div v-if="products.length > 0">
                ...
                <transition-group name="row" tag="div">
                    <div class="product-row" v-for="(product, index) in products" :key="product.id">
                        ...
                        <div>
                            <custom-input :name="'date_' + product.id" type="date" :rules="validateRules.date" v-model="product.date" placeholder="날짜 선택" />
                        </div>
                        <div>
                            <custom-input :name="'name_' + product.id" type="text" :rules="validateRules.name" v-model="product.name" placeholder="상품명 입력" />
                        </div>
                        <div class="price-container">
                            ...
                            <div class="price-input-wrap">
                                <custom-input :name="'price_' + product.id" type="number" :rules="(val) => validateRules.price(val, product.fullPrice)" v-model="product.price" placeholder="가격 입력" :is-price="true" suffix="원" :class="{ 'is-full-price': product.priceType === 'full' }" />
                            </div>
                        </div>
                        <div class="row-del">
                ...
                <div class="submit-area">
                    <button class="btn-submit" @click="onSubmit" style="background: var(--accent2);">
                        Vee-Validate validate() 전송
                    </button>
                </div>
            </div>
    ...
    setup() {
        const { ref } = Vue;
        const { useForm, validate: validateField } = VeeValidate;
        ...
        const onSubmit = async () => {
            ...
            products.value.forEach((p) => {
                promises.push(validateField(p.date, validateRules.date));
                promises.push(validateField(p.name, validateRules.name));
                promises.push(validateField(p.price, (val) => validateRules.price(val, p.fullPrice)));
            });

            const results = await Promise.all(promises);
            isValid = results.every(res => res.valid);

            if (isValid) {
                console.log('Manual Submission Success:', products.value);
                alert('Vee-Validate 엔진 수동 검증 성공!');
            } else {
                alert('검증 실패: 모든 필드를 확인해주세요.');
            }
        };
